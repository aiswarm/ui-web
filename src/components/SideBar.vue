...
div
  ...
  ul(v-if="groups.length")
    li(v-for="group in groups" :key="group.name" @click="selectChat(group.name, 'group')") {{ group.name }}
    ...
    ul(v-if="agents.length")
      li(v-for="agent in agents" :key="agent.name" @click="selectChat(agent.name, 'agent')") {{ agent.name }}
